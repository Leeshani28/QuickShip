...
$statusResult = $vehicleObj->getVehicleType();

$fpdf = new FPDF("P","mm","A4");
$fpdf->SetTitle("Vehicle Category Report");
...
$fpdf->Cell(0,12,"VEHICLE CATEGORY REPORT",0,1,"C");
...
$fpdf->Cell(0,10,"VEHICLE CATEGORY REPORT",0,1,"C",true);
...
$fpdf->Cell($statusWidth,10,"Vehicle Category",1,0,"C",true);
...
while($row=$statusResult->fetch_assoc())
{
    ...
    $fpdf->Cell($statusWidth,9,$row["vehicle_type"],1,0);

    $fpdf->Cell($countWidth,9,$row["total_vehicles"],1,1,"C");

    $total += $row["total_vehicles"]; //$total = $total + $row["total_vehicles"];
