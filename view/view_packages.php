<?php

include_once '../commons/session.php';

include_once '../model/package_model.php';

//get user information from session
$userrow=$_SESSION["user"];

$packageObj = new Package();

$packageResult = $packageObj->getAllPackages();


?>

<html>
    <head>
        <?php include_once "../includes/bootstrap_css_includes.php"?>
        <link rel="stylesheet" type="text/css" href="../bootstrap/css/dataTables.bootstrap.min.css">
    </head>
    <body>
        <div class="container">
            <?php $pageName="PACKAGE MANAGEMENT" ?>
            <?php 
            if($userrow["user_role"]==1){
            include_once "../includes/header_row_includes_admin.php";
            } else {
                include_once "../includes/header_row_includes2.php";
            }
            ?>
            
            <div class="row">


                <nav aria-label="breadcrumb">

                    <ol class="breadcrumb">
                        <button type="button" class="btn btn-primary" onclick="history.back()"> ← Back </button>

                        <li class="breadcrumb-item">
                            <a href="dashboard_new.php">Dashboard</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">
                            <a href="package.php">Package Management</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">
                            View Packages
                        </li>
                    </ol>
                </nav>

            <div class="col-md-10">
            <a href="view_packages.php"><button type="button" class="btn btn-success btn-lg"><span class="glyphicon glyphicon-search"></span> View Packages</button></a>
            <a href="generate_package_reports.php"><button type="button" class="btn btn-warning btn-lg"><span class="glyphicon glyphicon-book"></span> Generate Package Reports</button></a>
            </div>


            </div>

            <div class="row">
                &nbsp
            </div>
            
            <div class="col-md-12">
                <?php 
                if(isset($_GET["msg"])){
                    $msg= base64_decode($_GET["msg"]);

                
                
                ?>
                <div class="row" id="msg">
                    <div class="alert alert-success">
                        <?php echo $msg ?>

                    </div>

                </div>

                <?php 
                }
                ?>
                <div class="row">
                    <div class="col-md-12">
                        <table class="table table-info table-striped" id="packagetable">
                            <thead>
                                <tr>
                                    <th>Package ID</th>
                                    <th>Order ID</th>
                                    <th>Package Type</th>
                                    <th>Weight(kg)</th>
                                    <th>Fragile Item</th>
                                    <th>&nbsp;</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php 
                                while($packagerow=$packageResult->fetch_assoc()){

                                    $package_id=$packagerow["package_id"];
                                    $package_id=base64_encode($package_id);

                                    $order_id=$packagerow["order_id"];
                                    $order_id=base64_encode($order_id);


                                    // $status="Active";
                                    // if($packagerow["pkg_status"]==0){
                                    //     $status="De-Active";

                                    // }
                                
                                
                                
                                ?>
                                
                                <tr>
                                    <td>
                                        <?php 
                                        echo $packagerow["package_id"];
                                        
                                        ?>
                                    </td>
                                    <td>
                                    <?php 
                                        echo $packagerow["order_id"];
                                        
                                        ?>
                                    </td>
                                    <td>
                                    <?php 
                                        echo $packagerow["pkg_type"];
                                        
                                        ?>
                                    </td>
                                    <td>
                                    <?php 
                                        echo $packagerow["pkg_weight"];
                                        
                                        ?>
                                    </td>
                                    <td>
                                    <?php 
                                         echo $packagerow["fragile_item"] ? "Yes" : "No"; 
                                        
                                        ?>
                                    </td>
                                    
                                    <td>
                                        <a href="view_Package.php?package_id=<?php echo $package_id; ?>&order_id=<?php echo $order_id; ?>" class="btn btn-info">
                                            <span class="glyphicon glyphicon-search"></span>
                                            &nbsp;
                                            View
                                        </a>
                                        &nbsp;

                                        
                                        <!-- &nbsp;
                                        <?php 
                                            if($packagerow["pkg_status"]==0)
                                            {  
                                        
                                        
                                        ?>

                                        <a href="../controller/user_controller.php?status=activate&package_id=<?php echo $package_id; ?>" class="btn btn-success">
                                            <span class="glyphicon glyphicon-ok"></span>
                                            &nbsp;
                                            Activate
                                        </a>
                                        <?php 
                                            }   
                                        
                                        
                                        ?>
                                        &nbsp;
                                        <?php 
                                            if($packagerow["pkg_status"]==1)
                                            {  
                                        
                                        
                                        ?>

                                        <a href="../controller/user_controller.php?status=deactivate&package_id=<?php echo $package_id; ?>" class="btn btn-danger">
                                            <span class="glyphicon glyphicon-remove"></span>
                                            &nbsp;
                                            De-Activate
                                        </a>
                                        <?php 
                                            }

                                
                                        
                                        ?>
                                        &nbsp; -->                                       

                                    </td>
                                </tr>
                                <?php 
                                }
                                
                                ?>
                            </tbody>
                        </table>
                    </div>
                </div>


            </div>
        </div>
    </body>

    <div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog">
    
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
            <h4>Are you sure</h4>
        </div>
        <div class="modal-body">
        
            
            <div class="row">
                <div class="col-md-12" style="text-align:right;">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    <a href="../controller/user_controller.php?status=delete&package_id=<?php echo $package_id; ?>" class="btn btn-danger">
                                            
                                            &nbsp;
                                            Delete
                                        </a>
                </div>
            </div>
           
           
        

        </div>
        

      </div>
      
    </div>
  </div>

  
    <script src="../js/datatable/jquery-3.5.1.js"></script>
    <script src="../js/datatable/dataTables.bootstrap.min.js"></script>
    <script src="../js/datatable/jquery.dataTables.min.js"></script>
    <script src="../bootstrap/js/bootstrap.min.js"></script>
    <script src="../js/datatable/datatables.js"></script>

    <script>
        $(document).ready(function(){
            $("#packagetable").DataTable();

            




        });

        
    </script>
    <script>
    const msg = document.getElementById('msg');

    const delayTime = 3000;

    setTimeout(() => {
        msg.style.display = 'none';
    }, delayTime);
</script>
</html>